...
for (i in c(1:(length(N_vec) %/% 10))){
  print(sprintf("If N = %d then optimal nu is %d", N_vec[10*i], nu_max_each_N[10*i]))
}
...
library(plotly)
max_idx <- which(llhs == max(llhs), arr.ind = TRUE)
i_max <- max_idx[1] # Row index
j_max <- max_idx[2] # Col index

x_peak <- N_vec[i_max] 
y_peak <- nu_vec[j_max]
z_peak <- llhs[i_max, j_max]

# Plotly expects a list or simple vectors for x and y, and a matrix for z
plot_ly(x = N_vec, y = nu_vec, z = t(llhs)) %>%
  add_surface() %>%
  add_markers(x = x_peak, y = y_peak, z = z_peak, 
              marker = list(size = 5, color = 'red', symbol = 'cross'),
              name = "Argmax") %>%
  layout(title = "Interactive 3D Surface Plot",
