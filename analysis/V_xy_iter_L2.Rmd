---
title: "Investigate V after every iteration in SuSiE RSS (True L=2)"
author: "Dat Do"
date: "2025-09-08"
output:
...
In the experiment below, the causal SNP's position is randomly chosen between 1 and $J = 200$. Set $L = 3$ in SuSiE and in every iteration, we will look at three figures: (1) $\overline{r}$ after adding back $\ell$-th CS; (2) PIP got from Single Effect Model; (3) $\overline{r}$ after subtracting $R \overline{b}$. We consider In-sample, Out-of-sample, and Projected (our method) covariance matrices.
...
First we look at In-sample Covariance matrix.

```{r}
knitr::opts_chunk$set(echo = TRUE, fig.width=8, fig.height=7, dpi=150)

## seed present: 1 (not subtract enough), 8 (v increase after subtracting)
seed = 8
...
plot_r <- function(V, b){
  ...
  cols <- rep('black', J)
  cols[gamma] <- 'red'
  plot(V, pch=16, ylab='residual', col=cols)
}
...
```{r}
#### 4. R_{alpha}
alpha = 1 / max(abs(V_xy))^2
V_xx = alpha * tcrossprod(V_xy)
diag(V_xx) <- 1
sigma2 = 1
sigma02 = 1
L = 3
...
for (iter in (1:max_iter)){
  ...
  mtext(paste0("R alpha, Iteration ", iter), outer = TRUE, cex = 1.5, line = 1)
}
