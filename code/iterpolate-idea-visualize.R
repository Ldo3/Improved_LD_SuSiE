...
load('output/coverages_mat_multi_trait_L1.RData')
load('output/powers_mat_multi_trait_L1.RData')
load('output/number_CSs_mat_multi_trait_L1.RData')
load('output/number_SNPs_mat_multi_trait_L1.RData')

# load('output/coverages_mat_multi_trait_L2.RData')
# load('output/powers_mat_multi_trait_L2.RData')
# load('output/number_CSs_mat_multi_trait_L2.RData')
# load('output/number_SNPs_mat_multi_trait_L2.RData')
...
library(boot)
for (i in 1:4){
  m = list_mat[[i]][rows_all_ok, c(1:12)]
  m[m==-1] <- 0
  colnames(m) <- c('In-', 'Out-', 'L=1', 'reg.',
                   'P1(R)', 'P1(0)', 'P1(R/2)', 'P1(3R/4)',
                   'P10(R)', 'P10(0)', 'P10(R/2)', 'P10(3R/4)')
  means <- colMeans(m)
...
wrap_plots(plots, ncol = 2) +
  plot_annotation(
    title = "Performance when true L = 1"
  ) &
  theme(
