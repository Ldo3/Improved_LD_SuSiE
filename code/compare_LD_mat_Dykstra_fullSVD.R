...
for (seed in 1:num_reps){
  ...
  for (t in 1:4){
    beta_ = rep(0, J)
    causal_SNPs = sample(1:J, size=3, replace=FALSE)
    true_effect = c(2, -3, 2)
...
plots = list()
for (i in 1:4){
  m = list_mat[[i]][ , c(1, 2, 9, 3, 4, 5, 6, 7, 8)]
  m[m==-1] <- 0
  colnames(m) <- c('In-', 'Out-', 'Reg.', 'P1(R)', 'P5(R)', 'P20(R)', 'P1(0)', 'P5(0)', 'P20(0)')
  means <- colMeans(m)
  sds   <- apply(m, 2, sd)
  df <- data.frame(
...
save(coverages, file='output/coverages_mat_multi_trait.RData')
save(powers, file='output/powers_mat_multi_trait.RData')
save(no_CSs, file='output/number_CSs_mat_multi_trait.RData')
save(no_SNPs_CS, file='output/number_SNPs_per_CS_mat_multi_trait.RData')
